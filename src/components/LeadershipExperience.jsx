              ...
              <div className={`relative bg-gradient-to-br ${item.color} p-0.5 rounded-xl shadow-xl hover:scale-105 transition-transform duration-300`}>
                <div className="bg-black/80 backdrop-blur-md rounded-xl p-5 w-full h-full flex flex-col justify-between min-h-[300px]">
                  ...
                  <div className="flex items-center justify-center mb-4">
                    {item.logo ? (
                      <div className="w-24 h-16 bg-white rounded-md flex items-center justify-center p-3 overflow-hidden">
                        <img 
                          src={item.logo} 
                          alt={item.title}
                      ...
                      <div className="w-20 h-20 bg-white/10 rounded-lg flex items-center justify-center text-4xl">
                        {item.icon}
                      </div>
                    )}
                  </div>
                  ...
                  {item.hours && (
                    <div className="flex justify-center mb-3">
                      <div className="bg-white/20 backdrop-blur-sm px-2 py-1 rounded-full border border-white/30">
                        <span className="text-white font-bold text-sm">{item.hours}</span>
                        <span className="text-gray-300 text-xs ml-1">Hours</span>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="flex-grow flex flex-col justify-center">
                    ...
                    <p className="text-center text-blue-300 font-semibold text-sm mb-2">
                      {item.role}
                    </p>
                    {item.organization && (
